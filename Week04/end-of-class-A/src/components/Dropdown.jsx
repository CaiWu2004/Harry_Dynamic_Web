...
const Dropdown = (props) => {
  ...
  // useEffect(() => {}, []) = run once when the component mounts useEffect(() => {},[thing1, thing2]) = run on amount or wherever thing1 or thing2 updates/changes
  //useEffect(() => {}) = run after every single render (this is the same as accidentally calling a function i)

  //here we are adding a plain old vanilla js event listener so that we can close the
  // dropdown when the user clicks
  // outside of our dropdown component.

  //if the first argument (the function) returns another function,
  // that returned function get fired when
  // the component unmounts/destroyed/off screen. This is usually a cleanup function
  useEffect(() => {
    //add event listener
    ...
    //if useEffect returns a function, that's a cleanup function
    return () => {
